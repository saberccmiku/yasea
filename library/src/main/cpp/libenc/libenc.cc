...
libenc_setEncoderResolution(JNIEnv *env, jobject thiz, jint out_width, jint out_height) {
    ...
    if (nv21_frame.width != out_width || nv21_frame.height != out_height) {
        free(nv21_frame.data);
        nv21_frame.width = out_width;
        nv21_frame.height = out_height;
        nv21_frame.data = (uint8_t *) malloc(y_size * 3 / 2);
        nv21_frame.y = nv21_frame.data;
        nv21_frame.v = nv21_frame.y + y_size;
        nv21_frame.u = nv21_frame.v + y_size / 4;
    }
...
// For COLOR_FormatYUV420SemiPlanar
static jbyteArray libenc_RGBAToNV21(JNIEnv *env, jobject thiz, jbyteArray frame, jint src_width,
                                    jint src_height, jboolean need_flip, jint rotate_degree) {
    jbyte *rgba_frame = env->GetByteArrayElements(frame, NULL);
    ...
    int ret = ConvertFromI420(i420_scaled_frame.y, i420_scaled_frame.width,
                              i420_scaled_frame.u, i420_scaled_frame.width / 2,
                              i420_scaled_frame.v, i420_scaled_frame.width / 2,
                              nv21_frame.data, nv21_frame.width,
                              nv21_frame.width, nv21_frame.height,
                              NV21_COLOR_FMT);
    if (ret < 0) {
        LIBENC_LOGE("ConvertFromI420 failure");
        return NULL;
    ...
    int y_size = nv21_frame.width * nv21_frame.height;
    jbyteArray nv21Frame = env->NewByteArray(y_size * 3 / 2);
    env->SetByteArrayRegion(nv21Frame, 0, y_size * 3 / 2, (jbyte *) nv21_frame.data);
...
static JNINativeMethod libenc_methods[] = {
        ...
        {"RGBAToNV21",           "([BIIZI)[B",            (void *) libenc_RGBAToNV21},
